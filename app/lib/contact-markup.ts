...
export function getContactMarkup() {
  const source = readFileSync(sourcePath, "utf8");
  const rootStart = source.indexOf('<div id="ct-root"');
  const contentStart = source.indexOf('<section id="ct-hero"');
  const footerStart = source.indexOf('<footer data-screen-label="Footer"');

  if (rootStart === -1 || contentStart === -1 || footerStart === -1) {
    throw new Error("Impossible d’extraire la maquette Contact.");
  }
  ...
  return imageSlotsToImages(markup)
    ...
    .replace(/\s+onSubmit="\{\{ onSubmit \}\}"/g, "")
    .replaceAll("opacity:{{ statusOpacity }}", "opacity:0")
